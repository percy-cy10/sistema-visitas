...
class VisitasController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('modulos.registrar-visita', ['oficinas' => $oficinas, 'sedes'=> $sedes, 'funcionario'=>$funcionario]);
    }
    ...
    public function store(Request $request)
    {
            ...
            'dni' => [ 'required','digits:8',
               function ($attribute, $value, $fail) {
                    if (Visitas::where('dni', $value)->where('estado', '2')->exists()) {
                        ...
                        if ($diffInMinutes < 3) {
                            $fail('Ya se registró una visita con este DNI en los últimos 3 minutos.');
                        }
                    }
                }
            ],
        ...
        $visita->fecha_y_hora = Carbon::now()->toDateTimeString();
        $nombrefuncionario = Funcionario::find($request->input('funcionario'))->first();
        $visita->empresa = strtoupper($request->input('empresa') ?? 'No tiene entidad');
        $visita->motivo = strtoupper($request->input('motivo'));
        $visita->funcionario_id = $nombrefuncionario->id;
        $visita->cargo = $request->input('cargo');
